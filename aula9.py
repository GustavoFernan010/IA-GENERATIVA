...
if uploaded_file is not None:
    # Tenta abrir a imagem com tratamento de erro para arquivos corrompidos
    try:
        # Convertemos para RGB para evitar erros com PNGs que possuem canal alpha (transparência)
        image = Image.open(uploaded_file).convert("RGB")
    except Exception as e:
        st.error(f"Não foi possível abrir a imagem enviada. Detalhe do erro: {e}")
        st.stop()
    ...
    if len(boxes) == 0:
        ...
        # Ordena os objetos detectados por nível de confiança (do maior para o menor)
